/* Remove breakable and collectible tiles from background image.

   ./remove_mutable_bg {metadata.png} {gray_bg.png} {output.png}

   This is meant to preprocess input images for use with remove_hidden_tiles.c,
   so that we preserve tiles that are behind mutable tiles.
*/
...
/* Set tile selection behavior via preprocessor macros. */
#ifdef INVERT_TILE_SELECTION
   #define PERMANENT_TILE_ACTION  1  /* Drop permanent. */
   #define MUTABLE_TILE_ACTION    0  /* Keep mutable. */
#else
   #define PERMANENT_TILE_ACTION  0  /* Keep permanent. */
   #define MUTABLE_TILE_ACTION    1  /* Drop mutable. */
#endif
...
/* Check annotation based on center and auxiliary center pixel colors.
   Returns nonzero if tile should be removed.                          */
static int IsRemovableTile(unsigned int pixel, unsigned int auxiliary_pixel)
{
   /* Ignore all transparent pixels. */
   if( (pixel & 0xff000000) == 0 )
      return PERMANENT_TILE_ACTION;
       ...
       ((pixel & 0xff0000) < 0x800000) )
   {
      ...
      return (auxiliary_pixel & 0xffffff) == 0 ? MUTABLE_TILE_ACTION
                                               : PERMANENT_TILE_ACTION;
   }

   /* Ignore chain reaction tiles (cyan). */
   if( ((pixel & 0x0000ff) < 0x000080) &&
       ((pixel & 0x00ff00) > 0x007f00) &&
       ((pixel & 0xff0000) > 0x7f0000) )
   {
      return PERMANENT_TILE_ACTION;
   }

   /* Ignore breakable chain reaction tiles (magenta). */
   if( ((pixel & 0x0000ff) > 0x00007f) &&
       ((pixel & 0x00ff00) < 0x008000) &&
       ((pixel & 0xff0000) > 0x7f0000) )
   {
   ...
   /* Remaining annotations are collectible (green) or throwable (yellow). */
   return MUTABLE_TILE_ACTION;
}

/* Remove tiles that are breakable or collectible. */
                                ...
                                png_bytep pixels)
{
   ...
   for(y = 0; y < height; y += TILE_SIZE)
   {
      ...
      {
         if( IsRemovableTile(READ_RGBA(metadata_pixels,
